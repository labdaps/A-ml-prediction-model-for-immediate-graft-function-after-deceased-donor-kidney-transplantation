  ...
  """
    ...
    """
        ...
        for i in data.drop(self.target, axis=1).columns:
            if (
                    data[i].dtypes == "int64"
            ):  # ((data[i].dtypes == 'int64') & (len(data[i].unique())>2))
    ...
    """
        ...
        for (
            ...
            data[i] = data[i].astype(
                self.learent_dtypes[self.learent_dtypes.index == i]
            )
  ...
  """
    ...
    def transform(self, dataset, y=None):
        ...
        for i in self.time_feature:
            ...
            if "is_month_start" in self.list_of_features_o:
                data[i + "_is_month_start"] = [
                    1 if datetime.date(r).day == 1 else 0 for r in data[i]
                ]
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if len(self.time_feature) == 0:
            self.time_feature = [
                i for i in data.columns if data[i].dtype == "datetime64[ns]"
            ]
  ...
  """
    ...
    def fit(self, dataset, y=None):
        ...
        if len(data.select_dtypes(include=("object")).columns) > 0:
            ...
            try:
                self.data_columns = pd.get_dummies(
  ...
  """
    ...
    def fit_transform(self, dataset, y=None):
        ...
        for k in self.ph.index:
            ...
            self.ph.iloc[k, 2] = metrics.calinski_harabaz_score(
                data_t1, self.k_object.labels_
            )
...
class Reduce_Cardinality_with_Clustering(BaseEstimator, TransformerMixin):
    ...
    - Highly recommended to run the DataTypes_Auto_infer class first
      Args:
          target_variable: target variable (integer or numerical only)
          categorical_feature: list of features on which clustering  is to be applied / cardinality to be reduced
          check_clusters_upto: to determine optimum number of kmeans clusters, set the uppler limit of clusters
  """
...
class DFS_Classic(BaseEstimator, TransformerMixin):
  ...
  """
    ...
    def __init__(
        ...
        self.top_n_correlated = (
            top_features_to_pick_percentage
        )  # (this will be 1- top_features , but handled in the Advance_feature_selection )
        self.ml_usecase = ml_usecase
        self.random_state = random_state
        self.subclass = subclass
    ...
    def transform(self, dataset, y=None):
        ...
        if "multiply" in self.interactions:
            ...
            data_multiply = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore").mul(
                        col[1], axis="index"
                    )
            ...
            data_multiply.columns = [
                "_multiply_".join([i, j])
                for j in data.drop(self.target, axis=1, errors="ignore").columns
                for i in data.drop(self.target, axis=1, errors="ignore").columns
            ]
        ...
        if "divide" in self.interactions:
            ...
            data_divide = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore")[
                        self.numeric_columns
                    ].div(col[1], axis="index")
                    for col in data.drop(self.target, axis=1, errors="ignore")[
                    self.numeric_columns
                ].iteritems()
                ],
                axis=1,
            ...
            data_divide.columns = [
                "_divide_".join([i, j])
                for j in data.drop(self.target, axis=1, errors="ignore")[
                    self.numeric_columns
                ].columns
                for i in data.drop(self.target, axis=1, errors="ignore")[
                    self.numeric_columns
                ].columns
            ]
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if "multiply" in self.interactions:
            data_multiply = pd.concat(
                [
                    data.drop(self.target, axis=1).mul(col[1], axis="index")
                    for col in data.drop(self.target, axis=1).iteritems()
                ],
                axis=1,
            )
            data_multiply.columns = [
                "_multiply_".join([i, j])
                for j in data.drop(self.target, axis=1).columns
                for i in data.drop(self.target, axis=1).columns
            ]
        ...
        if "divide" in self.interactions:
            data_divide = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].div(
                        col[1], axis="index"
                    )
                    for col in data.drop(self.target, axis=1)[
                    self.numeric_columns
                ].iteritems()
                ],
                axis=1,
            ...
            data_divide.columns = [
                "_divide_".join([i, j])
                for j in data.drop(self.target, axis=1)[self.numeric_columns].columns
                for i in data.drop(self.target, axis=1)[self.numeric_columns].columns
            ]
            # we dont need columns that are self interacted
            col = [
                "_divide_".join([i, j])
        ...
        if "add" in self.interactions:
            data_add = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].add(
                        col[1], axis="index"
                    )
                    for col in data.drop(self.target, axis=1)[
                    self.numeric_columns
        ...
        if "subtract" in self.interactions:
            data_substract = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].sub(
                        col[1], axis="index"
                    )
                    for col in data.drop(self.target, axis=1)[
                    self.numeric_columns
...
def Preprocess_Path_One(
        ...
        features_todrop=[],
        display_types=True,
        imputation_type="simple imputer",
        ...
        target_transformation_method="bc",
  ...
  """
    #bug fix:    replacing infinite values with NaN values, because they are practically useless and should be imprinted later
    ...
    if apply_cardinality_reduction == True and cardinal_method == "cluster":
        cardinality = Reduce_Cardinality_with_Clustering(
            target_variable=target_variable,
            categorical_feature=cardinal_features,
    ...
    if (target_transformation == True) and (ml_usecase == "regression"):
        global pt_target
        pt_target = Target_Transformation(
            target=target_variable, function_to_apply=target_transformation_method
        )
    else:
        pt_target = Empty()
...
class ResamplerTransformer(BaseEstimator, TransformerMixin):
    """
    - Applies resampling strategies into training dataset

      Args:
        resampling_method = string, name of the resampling method
  ...
  """
    ...
    def __init__(self, resampling_method):
        ...
        if resampling_method == "oss":
            from imblearn.under_sampling import OneSidedSelection
            self.resampler = OneSidedSelection(random_state=42, n_jobs=-1)
        if resampling_method == "enn":
            from imblearn.under_sampling import EditedNearestNeighbours
            self.resampler = EditedNearestNeighbours(n_jobs=-1)
        if resampling_method == "random_under":
            from imblearn.under_sampling import RandomUnderSampler
            self.resampler = RandomUnderSampler(random_state=42)
        if resampling_method == "smoteenn":
            from imblearn.combine import SMOTEENN
            self.resampler = SMOTEENN(random_state=42, n_jobs=-1)
        if resampling_method == "smote":
            from imblearn.over_sampling import SMOTE
            self.resampler = SMOTE(random_state=42, n_jobs=-1)
        if resampling_method == "random_over":
    ...
    def fit(
        ...
        data = dataset.copy()
        y = y.copy()
        self.resampler.fit(data, y)

    def fit_transform(self, dataset, y):
        data = dataset.copy()
        y = y.copy()
        self.fit(data, y)
        return self.resampler.fit_resample(data, y)
